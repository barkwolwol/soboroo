        ...
        <div class="row">
          <div class="col-lg-12">
              <div class="banner-heading">
                <h1 class="banner-title">온라인 모임</h1>
  ...
  <div class="container">
    <div style="text-align: right">
      <c:if test="${ not empty loginUser }">
        <a class="btn btn-primary" href="enrollOnlineGroupOne.go">새로운 모임</a>
      </c:if>
    </div>
  </div>
</section>
...
<div class="pagination_wrap">
    <div class="pagination">
      <c:choose>
        ...
        <c:otherwise>
            <a href="onList.go?tableNo=2&cpage=${ pi.currentPage - 1 }" class="btn_prev">이전</a>
        </c:otherwise>
      </c:choose>
      <c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
        <c:choose>
          <c:when test="${ p eq pi.currentPage }">
            <a href="onList.go?tableNo=2&cpage=${ p }" class="page_move active disabled">${ p }</a>
          </c:when>
          <c:otherwise>
              <a href="onList.go?tableNo=2&cpage=${ p }" class="page_move active">${ p }</a>
          </c:otherwise>
        </c:choose>
      </c:forEach>
      <c:choose>
        ...
        <c:otherwise>
          <a href="onList.go?tableNo=2&cpage=${ pi.currentPage + 1 }" class="btn_next">다음</a>
        </c:otherwise>
        </c:choose>
    </div>
</div>
