...
<body>
	...
	<div class="body-inner">
		...
		<div id="banner-area" class="banner-area"
			style="background-image: url(resources/images/dog.png)"
			; 
						...
						<div class="post" id="myGroup">
							...
							<div class="post-body">
								<div class="entry-header">
									<h2 class="entry-title">작성한 커뮤니티 댓글</h2>
								</div>
								<!-- header end -->
  ...
  <c:otherwise>
    ...
    <div class="pagination">
      <c:choose>
        ...
        <c:otherwise>
            <a href="communityReplyList.my?cpage=${ pi.currentPage - 1 }" class="btn_prev">이전</a>
        </c:otherwise>
      </c:choose>
      <c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
        <c:choose>
          <c:when test="${ p eq pi.currentPage }">
            <a href="communityReplyList.my?cpage=${ p }" class="page_move active disabled">${ p }</a>
          </c:when>
          <c:otherwise>
              <a href="communityReplyList.my?cpage=${ p }" class="page_move active">${ p }</a>
          </c:otherwise>
        </c:choose>
      </c:forEach>
      <c:choose>
        ...
        <c:otherwise>
          <a href="communityReplyList.my?cpage=${ pi.currentPage + 1 }" class="btn_next">다음</a>
        </c:otherwise>
        </c:choose>
    </div>
</div>
