 ...
 --%>
    ...
    <div class="pagination">
      <c:choose>
        ...
        <c:otherwise>
            <a href="selectMyGroup.my?cpage=${ pi.currentPage - 1 }" class="btn_prev">이전</a>
        </c:otherwise>
      </c:choose>
      <c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
        <c:choose>
          <c:when test="${ p eq pi.currentPage }">
            <a href="selectMyGroup.my?cpage=${ p }" class="page_move active disabled">${ p }</a>
          </c:when>
          <c:otherwise>
              <a href="selectMyGroup.my?cpage=${ p }" class="page_move active">${ p }</a>
          </c:otherwise>
        </c:choose>
      </c:forEach>
      <c:choose>
        ...
        <c:otherwise>
          <a href="selectMyGroup.my?cpage=${ pi.currentPage + 1 }" class="btn_next">다음</a>
        </c:otherwise>
        </c:choose>
    </div>
</div>
